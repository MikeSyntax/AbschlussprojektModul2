...
class BreakingTheBank(tableNumber: Int): Casino(tableNumber){
    ...
    fun breakingBank1(rouletteTable: RouletteTable, players: MutableList<CasinoPlayer>, croupiers: MutableList<CasinoEmployees>) {
        if (rouletteTable.bankTable <= 0) {  // Wenn der Bankbetrag kleiner oder gleich null ist
            ...
            croupiers.removeAll(croupiers)   // Entferne alle Croupier von Tisch 1
            ...
            println("     Die Bank an Roulette Tisch${FontColors.COLOREND.type} ${FontColors.BLUE.type}$tableNumber${FontColors.COLOREND.type} ${FontColors.YELLOW.type}wurde gesprengt\n" +
                    "       und das Spiel ist an diesem Tisch zu Ende\n" +
                    "    alle Spieler und Croupiers verlassen den Tisch")
            println("------------------------------------------------------${FontColors.COLOREND.type}")
        }
    }

    // Funktion, um zu prüfen, ob die Bank am High Roller Tisch 2 pleite ist
    fun breakingBank2(rouletteTable: RouletteTable, playersHighRoller: MutableList<CasinoPlayer>, highRollerCroupiers: MutableList<CasinoHighRollerEmployees>) {
        if (rouletteTable.bankTable <= 0) {  // Wenn der Bankbetrag kleiner oder gleich null ist
            ...
            highRollerCroupiers.removeAll(highRollerCroupiers)// Entferne alle High Roller Croupiers von Tisch 2
            println("${FontColors.YELLOW.type}------------------------------------------------------")
            println("     Die Bank an Roulette Tisch${FontColors.COLOREND.type} ${FontColors.BLUE.type}$tableNumber${FontColors.COLOREND.type} ${FontColors.YELLOW.type}wurde gesprengt\n" +
                    "       und das Spiel ist an diesem Tisch zu Ende\n" +
                    "    alle Spieler und Croupiers verlassen den Tisch")

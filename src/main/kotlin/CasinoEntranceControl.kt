...
class CasinoEntranceControl(tableNumber: Int = 1): Casino(tableNumber) {
    ...
    fun entranceControl(player: CasinoPlayer, players: MutableList<CasinoPlayer>){
        ...
        if (player.age >= 21){
            ...
            println("${player.name} Einlass ${FontColors.GREEN.type}gewährt${FontColors.COLOREND.type}")
            ...
            // und nicht rein darf
            println("${FontColors.RED.type}Tut uns leid, aber ${FontColors.COLOREND.type}${player.name} ${FontColors.RED.type}du bist zu jung und kommst nicht rein${FontColors.COLOREND.type}")
